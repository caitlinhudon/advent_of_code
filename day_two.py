...
df[["min", "max"]] = df['n_range'].str.split(pat='-', expand=True)
df['min'] = df['min'].astype(int)
df['max'] = df['max'].astype(int)
df['letter_count'] = df.apply(lambda x: x.password.count(x.letter), axis=1)
df['valid'] = np.where((df['letter_count'] >= df['min']) & (df['letter_count'] <= df['max']), 1, 0)

answer_part_one = df['valid'].sum()
